...
import { getoneBrandProfile, getallCollectionbyBrand } from "../utils/useApi";
...
const BrandProfile = () => {
    const { data: profileData, isLoading: profileLoading, isError: profileError } = getoneBrandProfile();
    const { data: collectionData, isLoading: collectionLoading, isError: collectionError } = getallCollectionbyBrand();
  
    console.log(collectionData);
  
    if (profileLoading || collectionLoading) return <h1>Loading</h1>;
    if (profileError || collectionError) return <h1>Error</h1>;
            ...
            <div className="h-fit min-h-full w-full px-12 py-7">
                ...
                <div className=" relative flex w-full flex-col items-center justify-center p-10">
                    <div className="relative">
                        <img src={profileData.cover } className="bg-container  md: md: h-[355px] w-[1040px] rounded-[5px] bg-Liver bg-opacity-10 brightness-50" />
                    ...
                    <div className="relative top-[-70px] flex flex-col items-center justify-center">
                        <img src={profileData.image} className="  bottom-[-40px] h-[120px] w-[120px] rounded-full border-2 border-white"
                         />
                    ...
                    <p className=" font-['SF Pro Display'] bottom-[50px] w-fit text-center text-[28px] font-semibold text-VanDyke md:absolute md:bottom-[67px]">{profileData.fullName}</p>
                ...
                <IoIosAddCircleOutline
                     className="mb-5 cursor-pointer  rounded-[150px] bg-Liver text-black" size={30}
                />
